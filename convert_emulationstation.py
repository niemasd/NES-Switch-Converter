...
assert isdir(INDIR), "Non-existant input folder: %s" % argv[1]
assert isfile(GAMELIST), "Non-existant gamelist: %s" % GAMELIST
assert not isdir(OUTDIR), "Output folder exists: %s" % argv[2]

# perform conversion
gamelist = parse(GAMELIST).getroot()
progbar = Bar('Processed', max=len(gamelist.findall('game')))
mkdir(OUTDIR)
db = open('%s/lclassics.titlesdb' % OUTDIR, 'w')
db.write('{\n\t"titles": [')
for i,game in enumerate(gamelist):
    game_data = {attr.tag:attr.text for attr in game}
    ...
    GAMEDIR = '%s/%s' % (OUTDIR, game_data['name'])
    mkdir(GAMEDIR)
    rom = open('%s/%s.nes' % (GAMEDIR, game_data['name']), 'wb')
    ...
    rom.close()
    cover_orig = Image.open('%s/%s' % (INDIR, game_data['image'][2:]))
    with NamedTemporaryFile() as cover_tga:
        cover_orig.save(cover_tga, format='tga')
        cover_tga.flush()
        call([args.NvnTexpkg, '-i', cover_tga.name, '--mip-filter', 'box', '--minmip', '5', '-f', 'rgba8', '-o', '%s.xtx'%cover_tga.name])
        cover = open('%s/%s.xtx.z' % (GAMEDIR, game_data['name']), 'wb')
        cover.write(compress(open('%s.xtx'%cover_tga.name, 'rb').read(), 9))
        cover.close()
    if i != 0:
        db.write(',')
    db.write('\n\t\t{\n')
    db.write('\t\t\t"sort_title": "%s",\n' % game_data['name'].lower().replace("'",''))
    db.write('\t\t\t"publisher": "%s",\n' % game_data['publisher'])
    db.write('\t\t\t"code": "%s",\n' % game_data['name'])
    db.write('\t\t\t"rom": "/titles/%s/%s.nes",\n' % (game_data['name'], game_data['name']))
    db.write('\t\t\t"copyright": "%s",\n' % game_data['developer'])
    db.write('\t\t\t"title": "%s",\n' % game_data['name'])
    db.write('\t\t\t"volume": 74,\n')
    db.write('\t\t\t"release_date": "%s",\n' % convert_date(game_data['releasedate']))
    db.write('\t\t\t"players_count": %d,\n' % game_data['players'])
    db.write('\t\t\t"rom": "/titles/%s/%s.xtx.z",\n' % (game_data['name'], game_data['name']))
